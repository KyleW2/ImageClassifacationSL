...
if __name__ == "__main__":
    ...
    def runTests(ptron: Perceptron, test_data: list, label: int, number_of_tests: int, number_of_weight_updates: int) -> None:
        for j in range(0, number_of_tests):
            ...
            # Classify some tests
            f = open(f"results/perceptrons/airplane big/ptron_label_{label}_weights_iterated_{(j+1) * number_of_weight_updates}_times.csv", "w")
    ...
    runTests(airplanes_ptron, airplanes_test, 0, 2500, 1)
